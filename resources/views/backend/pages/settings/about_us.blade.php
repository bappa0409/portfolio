        ...
        <form id="terminalForm" class="mt-4" @submit.prevent="saveTerminal()">
            ...
            <div class="rounded-md border border-white/10 bg-slate-950/30 p-4 space-y-4">
                ...
                <div x-data="window.tagInput({
                    ...
                    <p class="mt-1 text-[11px] text-red-300 font-mono" x-show="error || err('terminal.stack')"
                        x-text="error || err('terminal.stack')"></p>
                </div>
        ...
        <form id="tagsForm" class="mt-4" @submit.prevent="saveTags()">
            ...
            class="rounded-md border border-white/10 bg-slate-950/30 p-4">
                ...
                <p class="mt-1 text-[11px] text-red-300 font-mono"
                    x-show="error || err('tags')"
                    x-text="error || err('tags')"></p>
            </div>
        </form>
    </section>
        ...
        <form id="educationForm" class="mt-4" @submit.prevent="saveEducation()">
            ...
            <div x-data="{ items: @js($education ?? []) }"
                ...
                <template x-for="(e,i) in items" :key="'err-'+i">
                    <div class="mb-2">
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`education.${i}.title`)"></p>
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`education.${i}.year`)"></p>
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`education.${i}.note`)"></p>
                    </div>
                </template>
            </div>
        </form>
            ...
            <div x-data="{ items: @js($training ?? []) }"
                ...
                <template x-for="(t,i) in items" :key="'err-'+i">
                    <div class="mb-2">
                        ...
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`training.${i}.institute`)"></p>
                ...
                <template x-for="(x,i) in items" :key="i">
                    <div class="mb-4 rounded-md border border-white/10 bg-slate-950/20 p-4">
                        ...
                        <div class="mt-3" x-data="{ tasks: x.tasks ?? [] }">
                            ...
                            <p class="mt-1 text-[11px] text-red-300 font-mono" x-text="err(`experience.${i}.tasks`)"></p>
                        </div>

                        <div class="mt-2">
                            <p class="text-[11px] text-red-300 font-mono" x-text="err(`experience.${i}.role`)"></p>
                            <p class="text-[11px] text-red-300 font-mono" x-text="err(`experience.${i}.company`)"></p>
                            <p class="text-[11px] text-red-300 font-mono" x-text="err(`experience.${i}.period`)"></p>
                        </div>
                    </div>
                </template>
            </div>
