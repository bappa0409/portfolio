                    ...
                    <div class="grid md:grid-cols-2 gap-3">
                        <template x-for="(m,i) in minis" :key="i">
                            <div>
                                ...
                                {{-- ✅ fix: nested scope --}}
                                <p class="mt-1 text-[11px] text-red-300 font-mono" x-text="err(`hero.mini_stats.${i}.value`)"></p>
                                <p class="mt-1 text-[11px] text-red-300 font-mono" x-text="err(`hero.mini_stats.${i}.label`)"></p>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            <div class="md:col-span-2 grid md:grid-cols-12 gap-4 items-start">
                ...
                <div class="md:col-span-3 flex flex-col items-center">
                    @if(data_get($settings->hero,'profile_image'))
                        ...
                        <img
                            src="{{ Storage::url(data_get($settings->hero,'profile_image')) }}"
                ...
                <template x-for="(w,i) in items" :key="'err-'+i">
                    <div class="mb-2">
                        <p class="text-[11px] text-red-300 font-mono"
                            x-text="err(`why_choose_me.${i}.icon`)"></p>
                        <p class="text-[11px] text-red-300 font-mono"
                            x-text="err(`why_choose_me.${i}.title`)"></p>
                        <p class="text-[11px] text-red-300 font-mono"
                            x-text="err(`why_choose_me.${i}.desc`)"></p>
                    </div>
                </template>
        ...
        <form id="processForm" class="mt-4" @submit.prevent="saveProcess()">
            ...
            <div x-data="{ items: @js($settings->process ?? []) }"
                ...
                <template x-for="(p,i) in items" :key="'err-'+i">
                    <div class="mb-2">
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`process.${i}.step`)"></p>
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`process.${i}.title`)"></p>
                        <p class="text-[11px] text-red-300 font-mono" x-text="err(`process.${i}.desc`)"></p>
                    </div>
                </template>
