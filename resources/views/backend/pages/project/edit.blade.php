...
@section('content')
    <div class="flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-300">Update website projects from here.</p>
        </div>
        <a href="{{ route('admin.project.index') }}" class="text-xs font-mono text-emerald-200 hover:text-emerald-100">← Back</a>
    </div>
    ...
    <div class="mt-4 rounded-md glass cyber-glow p-6 relative">
        ...
        <form method="POST" action="{{ route('admin.project.update', $project->id) }}"
              class="mt-6 grid md:grid-cols-2 gap-5" enctype="multipart/form-data">
            @csrf
            ...
            <div class="md:col-span-2">
                ...
                {{-- existing main image preview (optional) --}}
                @if(!empty($project->image))
                    <div class="mt-2 rounded-md overflow-hidden border border-white/10 bg-white/5">
                        <img src="{{ asset('images/projects/'.$project->image) }}" class="w-full h-40 object-cover" alt="">
                    </div>
                    <p class="mt-1 text-[11px] text-slate-500 font-mono">Current main image: {{ $project->image }}</p>
                @endif
            ...
            <div class="md:col-span-2 grid sm:grid-cols-2 gap-4">
                <label class="flex items-center gap-2 text-xs font-mono text-slate-300">
                    <input type="checkbox" name="visibility" value="1"
                           @checked(old('visibility', $project->visibility))
            ...
            @php
                ...
                if ($featuresText === null) {
                    $featuresArr = is_array($project->features) ? $project->features : [];
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="features" rows="6"
                    class="mt-2 w-full rounded-md border border-white/10 bg-slate-950/40 px-3 py-2 text-white"
                    placeholder="Role based access&#10;Reports&#10;API integration">{{ $featuresText }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                {{-- existing gallery thumbs (optional) --}}
                @php $gal = is_array($project->gallery) ? $project->gallery : []; @endphp
                @if(count($gal))
                    <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                        @foreach($gal as $g)
                            <div class="rounded-md border border-white/10 bg-white/5 overflow-hidden">
                                <img src="{{ asset('images/projects/'.$g) }}" class="w-full h-20 object-cover" alt="">
                                <div class="px-2 py-1 text-[10px] font-mono text-slate-400 truncate" title="{{ $g }}">{{ $g }}</div>
                            </div>
                        @endforeach
                    </div>
                    <p class="mt-1 text-[11px] text-slate-500 font-mono">
                        Upload new images to append. (Controller can also support replace if you want.)
                    </p>
                @endif
