    ...
    <div class="grid md:grid-cols-[7fr_3fr] items-center gap-10">
        ...
        <div>
            ...
            <p class="mt-4 text-lg text-slate-300">
                I build Laravel web apps, APIs, admin panels, and ecommerce solutions for businesses and startups.
            </p>

            <div class="mt-7 flex gap-3">
                <a href="{{ route('contact') }}"
                   class="rounded-xl bg-blue-500 hover:bg-blue-400 px-5 py-3 font-semibold text-slate-950">
                    Hire Me
                </a>

                <a href="{{ route('projects') }}"
                   class="rounded-xl border border-white/15 hover:border-white/30 px-5 py-3 font-semibold text-white">
                    View Projects
                </a>
            </div>
        ...
        <div class="rounded-xl border border-white/10 bg-white/5 p-4 text-white">
            ...
            <div class="flex justify-center mb-3">
                <img
                    ...
                    class="max-h-[300px] rounded-xl object-cover shadow-lg"
                >
            </div>
            ...
            <div class="mt-3">
                <a href="{{ route('contact') }}"
                   class="block text-center rounded-xl bg-white text-slate-950 font-semibold px-4 py-3 hover:bg-slate-100">
                    Get a Free Quote
                </a>
            </div>
        </div>
...
<section class="mx-auto max-w-6xl px-4 py-10">
    ...
    <div class="mt-6 grid md:grid-cols-3 gap-5">
        @foreach ($services as $s)
            <div class="rounded-2xl border border-white/10 bg-white/5 p-5 hover:border-white/20 transition">
                <h3 class="font-semibold text-white">{{ $s[0] }}</h3>
                <p class="mt-2 text-slate-300 text-sm">{{ $s[1] }}</p>
            </div>
        @endforeach
    </div>
</section>
                ...
                <div class="p-5">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        @foreach ($p['stack'] as $tag)
                            <span class="text-xs rounded-full bg-slate-950/60 border border-white/10 px-3 py-1 text-slate-200">
                                {{ $tag }}
                            </span>
                        @endforeach
                    </div>
...
<!-- CTA -->
<section class="mx-auto max-w-6xl px-4 py-10">
    <div class="rounded-2xl border border-white/10 bg-gradient-to-br from-blue-500/15 to-white/5 p-8
